...
boost_pylib=$(basename /usr/lib/$(arch)-linux-gnu/libboost_python*.so)
boost_pylibname=${boost_pylib%.so}
boost_pyname=${boost_pylibname/lib/}

echo "** Download pycuda-2024.1.2 sources"
pushd $folder
if [ ! -f pycuda-2024.1.2.tar.gz ]; then
...
echo "** Build and install pycuda-2024.1.2"
CPU_CORES=$(nproc)
